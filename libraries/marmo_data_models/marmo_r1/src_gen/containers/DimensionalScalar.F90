...
! Generated with marmo:containers:DimensionalScalar version [object Object]
!
!------------------------------------------------------------------------------
!******************************************************************************
module class_marmo_containers_DimensionalScalar
    !---------------------------------------------------------------------------
    !use statements
...
contains
    ...
    pure subroutine destroy(this)
        class(DimensionalScalar), intent(inout) :: this
    ...
    pure subroutine default_initFromSingle(this)
        ...
        ! Set the default name of the object
        this%name = string('DimensionalScalar')
    ...
    subroutine save_HDF5_toExistingDataBase(this,groupIndex)
        ...
        errorj=h5a_setType(groupIndex,'marmo:containers:DimensionalScalar' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of marmo:containers:DimensionalScalar'
            call throw(io_exception(error_message))
            return
        end if
        ...
        ! Save property value
        !-------------------------------------------------------------------------
        errorj = H5A_WriteDouble(groupIndex, 'value' // c_null_char,this%value)
        if (H5A_IS_ERROR(errorj)) then
        ...
        errorj = h5a_setOrder(groupIndex,'value,label,unit,name,description' // c_null_c&
            &har)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of description'
            call throw(io_exception(error_message))
    ...
    subroutine load_HDF5_fromGroupIndex(this, groupIndex)
        ...
        ! Load property value
        !-------------------------------------------------------------------------
        error = H5A_ReadDouble(groupIndex, 'value' // c_null_char, this%value)
        if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of DimensionalScalar'        &
        ...
        if (error.ge.0) then
            ...
            if (sv.ne.0) then
                error=-1
                error_message = 'Error during loading of DimensionalScalar, error when trying to&
                    & allocate name for label'
    ...
    logical elemental function isValid(this)
        class(DimensionalScalar), intent(in) :: this
        isValid = .true.
...
!---------------------------------------------------------------------------
end module  class_marmo_containers_DimensionalScalar
!******************************************************************************