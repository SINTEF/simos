...
HDF5Save.prototype.saveDataToHDF5Handle = function(bl) {
	...
	cmd.push(this.gbl(bl+1) + 			'if ("order" in list(handle.attrs.keys())):');
	...
	for(var i = 0; i < propNum; i++) {
		...
		if (this.isAtomicType(prop.type)) {
			if(this.isArray(prop)){
				...
				cmd.push(this.gbl(bl+1) + 'pyds.hdf5.saveToHDF5HandleItem(self, handle, ' + this.stringify(prop.name) + ', "AtomicArray")' );
			 }
			 else{
				 /* single atomic type value */
				cmd.push(this.gbl(bl+1) + 'pyds.hdf5.saveToHDF5HandleItem(self, handle, ' + this.stringify(prop.name) + ', "AtomicSingle")' );
			 }
		}
		else {
			/*
			...
			if(this.isArray(prop) && this.isUngroup(prop)){
				/* array non-atomic type reference */
				 cmd.push(this.gbl(bl+1) + 'pyds.hdf5.saveToHDF5HandleItem(self, handle, ' + this.stringify(prop.name) + ', "NonAtomicArray", ungroup=True)' );
			}	 
		    else if(this.isArray(prop) && !(this.isUngroup(prop))){
				/* array non-atomic type reference */
				 cmd.push(this.gbl(bl+1) + 'pyds.hdf5.saveToHDF5HandleItem(self, handle, ' + this.stringify(prop.name) + ', "NonAtomicArray")' );
				 
			 }
			 else{
				 /* single non-atomic type reference */
				 cmd.push(this.gbl(bl+1) + 'pyds.hdf5.saveToHDF5HandleItem(self, handle, ' + this.stringify(prop.name) + ', "NonAtomicSingle")' );
			 }
	...
	cmd.push(this.gbl(bl+1) + 	'if ("order" in list(handle.attrs.keys())):');
	cmd.push(this.gbl(bl+2) + 		'curOrders = [a.decode("ascii") for a in handle.attrs["order"]]');	
	cmd.push(this.gbl(bl+2) + 		'if ( len(list(curOrders)) > 0 ):');	
	cmd.push(this.gbl(bl+3) + 			'order = list(curOrders) + order');
	cmd.push(this.gbl(bl+1) + 	'handle.attrs["order"] = [a.encode("ascii") for a in order]' );
	
	cmd.push(this.gbl(bl+1) + 	'for key,val in self.OBJattrs.items():');
	...
	cmd.push(this.gbl(bl+2) + 		'if (not key in list(handle.attrs.keys())):');
	cmd.push(this.gbl(bl+3) + 			'handle.attrs[key] = val');
...
HDF5Save.prototype.saveToHDF5HandleItemAtomicArray = function(bl) {
    ...
    cmd.push(this.gbl(bl+3) +      'data = getattr(self,varName)' );  	
    cmd.push(this.gbl(bl+3) +      'if (self.isStringArray(data)):' );	
    cmd.push(this.gbl(bl+3) +           'shape = self.getShape(data)' );      
	cmd.push(this.gbl(bl+3) +      'handle[varName] = np.asarray(getattr(self,varName))' );
	cmd.push(this.gbl(bl+3) +      'self._saved[varName] = handle[varName].ref');	
	cmd.push(this.gbl(bl+1) +  'pass');
...
HDF5Save.prototype.saveToHDF5HandleItemNonAtomicSingle = function(bl) {
	...
	cmd.push(this.gbl(bl+3) + 			'if not(varName in list(handle.keys())):' );
	...
	//cmd.push(this.gbl(bl+3) + 			'if ("order" in list(dgrp.attrs.keys())):');
...
HDF5Save.prototype.saveToHDF5HandleItemNonAtomicArray = function(bl) {
	...
	cmd.push(this.gbl(bl) + 	'def _saveToHDF5HandleItemNonAtomicArray(self, handle, varName):');
	...
	for(var i = 0; i < propNum; i++) {
		...
		cmd.push(this.gbl(bl+2) + 			'maindgrp = None' );
		cmd.push(this.gbl(bl+2) + 			'if not(varName in list(handle.keys())):' );
		cmd.push(this.gbl(bl+3) + 				'maindgrp = handle.create_group(varName)' );
		cmd.push(this.gbl(bl+2) + 			'else:' );
		cmd.push(this.gbl(bl+3) + 				'maindgrp = handle[varName]' );
		...
		cmd.push(this.gbl(loopBlock.bl+1) +				'if not(item.name in list(maindgrp.keys())):' );
...
HDF5Save.prototype.saveToHDF5HandleItemNonAtomicArrayUngroup = function(bl) {
	...
	for(var i = 0; i < propNum; i++) {
		...
		cmd.push(this.gbl(bl+3) +		'if ("order" in list(maindgrp.attrs.keys())):');
