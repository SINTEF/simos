...
NOTE: In the file _config_ you can set up, add or change the folders where the models are and where the generated code is stored. File location: _simos/simos/config\pathConfig-org.js_. Example below <<#simos_config, pathConfig-org.js>>, see the new added outPath and modelPath for MAF2019.

'''
EXAMPLE: pathConfig-org.js
...
==== Alternative II: Store pathConfig.js locally

You can also make a copy of the `pathConfig.js` in the folder where you have your model and set the input and output path there.

See example in simos/example/:

* `simos/example/pathConfig.js`
* generate the package usuing the shell script `generate.sh` 

'''
EXAMPLE: pathConfig.js
...
'''
[source,js]
[#simos_config_local_run]
----
grob@SINTEFPC7475 MINGW64 /c/git/simos/simos/example (SIMOS-11-make-user-manual)
$ generate.sh
Starting SIMOS ...
