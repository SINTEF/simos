...
! a named entity
! Generated with marmo:basic:NamedEntity version [object Object]
!
!------------------------------------------------------------------------------
!******************************************************************************
module class_marmo_basic_NamedEntity
    !---------------------------------------------------------------------------
    !use statements
...
contains
    ...
    pure subroutine destroy(this)
        class(NamedEntity), intent(inout) :: this
    ...
    subroutine save_HDF5_toExistingDataBase(this,groupIndex)
        ...
        errorj=h5a_setType(groupIndex,'marmo:basic:NamedEntity' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of marmo:basic:NamedEntity'
            call throw(io_exception(error_message))
            return
        end if
        ...
        errorj = h5a_setOrder(groupIndex,'name,description' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of description'
            call throw(io_exception(error_message))
            return
    ...
    subroutine load_HDF5_fromGroupIndex(this, groupIndex)
        ...
        if (error.ge.0) then
            ...
            if (sv.ne.0) then
                error=-1
                error_message = 'Error during loading of NamedEntity, error when trying to alloc&
                    &ate name for name'
            ...
            deallocate(cc_a)
            if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of NamedEntity'        &
    ...
    logical elemental function isValid(this)
        class(NamedEntity), intent(in) :: this
        isValid = .true.
...
!---------------------------------------------------------------------------
end module  class_marmo_basic_NamedEntity
!******************************************************************************