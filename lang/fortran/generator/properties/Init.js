	...
	for(var i = 0; i < propNum; i++) {
		...
		if (this.isAtomic(prop) && this.isArray(prop)){
			...
			if (prop.type=='double'){
				...
				cmd.push(this.gbl(bl+2) + "errorj = H5A_WriteString(groupIndex, '" + prop.name + "' // c_null_char,this%" + prop.name + "%toChars() // c_null_char)");		
				cmd.push(this.gbl(bl+2) + "error=error+errorj");
				cmd.push(this.gbl(bl+1) + "end if");
			}	
		}
...
Init.prototype.loadH5Declaration = function(bl) {
	...
	cmd.push(this.gbl(bl) + "generic, public :: load_HDF5 => load_HDF5_DataBase, load_HDF5_fromOpenedDataBase");
	cmd.push(this.gbl(bl) + "procedure :: load_HDF5_DataBase");
	cmd.push(this.gbl(bl) + "procedure :: load_HDF5_fromOpenedDataBase");
