...
! String data.
! Generated with marmo:containers:SimpleString version [object Object]
!
!------------------------------------------------------------------------------
!******************************************************************************
module class_marmo_containers_SimpleString
    !---------------------------------------------------------------------------
    !use statements
...
contains
    ...
    pure subroutine destroy(this)
        class(SimpleString), intent(inout) :: this
    ...
    subroutine save_HDF5_toExistingDataBase(this,groupIndex)
        ...
        errorj=h5a_setType(groupIndex,'marmo:containers:SimpleString' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of marmo:containers:SimpleString'
            call throw(io_exception(error_message))
            return
        end if
        ...
        ! Save property value
        !-------------------------------------------------------------------------
        if (.not.(this%value%isEmpty())) then
            errorj = H5A_writeStringWithLength(groupIndex, 'value' // c_null_char,this%value%toChars() // c_null_char)
            if (H5A_IS_ERROR(errorj)) then
        ...
        errorj = h5a_setOrder(groupIndex,'value,name,description' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of description'
            call throw(io_exception(error_message))
            return
    ...
    subroutine load_HDF5_fromGroupIndex(this, groupIndex)
        ...
        ! Load property value
        !-------------------------------------------------------------------------
        error= H5A_getStringLength(groupIndex, 'value' // c_null_char,strSize)
        if (error.ge.0) then
            if (allocated(cc_a)) deallocate(cc_a)
            allocate(character :: cc_a(strSize+1),stat=sv)
            if (sv.ne.0) then
                error=-1
                error_message = 'Error during loading of SimpleString, error when trying to allo&
                    &cate name for value'
                call throw(illegal_state_exception(error_message%toChars()))
                return
            end if
            error = H5A_ReadStringWithLength(groupIndex, 'value' // c_null_char,cc_a)
            cc_a(strSize+1) = c_null_char
            this%value=String(cc_a)
            this%value=this%value%trim()
            deallocate(cc_a)
            if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of SimpleString'        &
                    + ' - failed to open property value'
            call throw(io_exception(error_message))
                return
            end if
        end if
    ...
    logical elemental function isValid(this)
        class(SimpleString), intent(in) :: this
        isValid = .true.
...
!---------------------------------------------------------------------------
end module  class_marmo_containers_SimpleString
!******************************************************************************