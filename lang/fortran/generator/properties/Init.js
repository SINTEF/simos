...
Init.prototype.saveH5 = function(bl) {
	...
	cmd.push(this.gbl(bl+1) + "integer :: dataBaseID");
	cmd.push(this.gbl(bl+1) + "type(String) :: defaultName");
	...
	cmd.push(this.gbl(bl+2) + "error = H5A_RemoveDatabase( fileName%toChars() // c_null_char)");
	...
	cmd.push(this.gbl(bl+1) + "");
	cmd.push(this.gbl(bl+1) + "! Save the current object");
	cmd.push(this.gbl(bl+1) + "defaultName='" + this.getTypeName() + "'");
	cmd.push(this.gbl(bl+1) + "call this%save_HDF5_toExistingDataBase(dataBaseID,defaultName)");

	cmd.push(this.gbl(bl+1) + "");
	cmd.push(this.gbl(bl+1) + "! Close the data base");
	cmd.push(this.gbl(bl+1) + "error = H5A_CloseDatabase(dataBaseID)");

	cmd.push(this.gbl(bl+1) + "");
	cmd.push(this.gbl(bl+1) + "! Error check");
	cmd.push(this.gbl(bl+1) + "if (H5A_IS_ERROR(error)) then");
	cmd.push(this.gbl(bl+2) + "write(*,*) 'Error during saving of "+ this.getTypeName() + "'");
	cmd.push(this.gbl(bl+1) + "end if");
	...
	cmd.push(this.gbl(bl+1) + "");
	cmd.push(this.gbl(bl+1) + "! Initialize the group from name (if it exists) or defaultName");
	cmd.push(this.gbl(bl+1) + "if (this%name%isEmpty()) then");
	cmd.push(this.gbl(bl+2) + "entityName=defaultName");
	cmd.push(this.gbl(bl+1) + "else");
	cmd.push(this.gbl(bl+2) + "entityName=this%name");
	cmd.push(this.gbl(bl+1) + "end if");
	cmd.push(this.gbl(bl+1) + "entityID = H5A_OpenOrCreateEntity(groupIndex,entityName%toChars() // c_null_char)");
	...
	for(var i = 0; i < propNum; i++) {
		...
		if (this.isAtomic(prop) && this.isArray(prop)){
			...
			if (this.isAllocatable(prop)){ 
				...
				if (prop.type=='double'){
					cmd.push(this.gbl(bl+2) + "error = H5A_WriteDoubleArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,this%" + prop.name + ")");
				}else if (prop.type=='integer'){
					cmd.push(this.gbl(bl+2) + "error = H5A_WriteIntArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,this%" + prop.name + ")");	
					...
					if (dimList.length > 1){
						...
						cmd.push(this.gbl(bl+2) + "error = H5A_WriteIntArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,logicalToIntArray)");
				...
				if (prop.type=='double'){
					cmd.push(this.gbl(bl+1) + "error = H5A_WriteDoubleArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,this%" + prop.name + ")");
				}else if (prop.type=='integer'){
					cmd.push(this.gbl(bl+1) + "error = H5A_WriteIntArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,this%" + prop.name + ")");	
				}else if (prop.type=='boolean'){
					if (dimList.length > 1){
						throw "save_hdf5 is not implemented for logical array of more than one dimension.";
					}else{
						...
						cmd.push(this.gbl(bl+1) + "error = H5A_WriteIntArray(entityID, '" + prop.name + "' // c_null_char," + dimList.length + ",diml,logicalToIntArray)");
			...
			if (prop.type=='double'){
				cmd.push(this.gbl(bl+1) + "error = H5A_WriteDouble(entityID, '" + prop.name + "' // c_null_char,this%" + prop.name + ")");
			}else if (prop.type=='integer'){
				cmd.push(this.gbl(bl+1) + "error = H5A_WriteInt(entityID, '" + prop.name + "' // c_null_char,this%" + prop.name + ")");	
			}else if (prop.type=='boolean'){
				...
				cmd.push(this.gbl(bl+1) + "end if");
				cmd.push(this.gbl(bl+1) + "error = H5A_WriteInt(entityID, '" + prop.name + "' // c_null_char,logicalToIntSingle)");	
			}else if (prop.type=='string'){
				cmd.push(this.gbl(bl+1) + "if (.not.(this%" + prop.name + "%isEmpty())) then");
				cmd.push(this.gbl(bl+2) + "error = H5A_WriteString(entityID, '" + prop.name + "' // c_null_char,this% " + prop.name + "%toChars())");		
				cmd.push(this.gbl(bl+1) + "end if");
			}	
		}
		else if (this.isSingle(prop) && (! this.isAtomic(prop))){
