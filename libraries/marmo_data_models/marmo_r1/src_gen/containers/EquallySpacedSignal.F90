...
! Generated with marmo:containers:EquallySpacedSignal version [object Object]
!
!------------------------------------------------------------------------------
!******************************************************************************
module class_marmo_containers_EquallySpacedSignal
    !---------------------------------------------------------------------------
    !use statements
...
!@@@@@ USER DEFINED USE End   @@@@@
    ...
    type   :: EquallySpacedSignal
        ...
        ! signal spacing.
        real(dp), public :: xdelta
        ! start of the signal input.
        real(dp), public :: xstart
...
contains
    ...
    pure subroutine destroy(this)
        class(EquallySpacedSignal), intent(inout) :: this
    ...
    pure subroutine default_initFromSingle(this)
        ...
        ! Set the default name of the object
        this%name = string('EquallySpacedSignal')
    ...
    subroutine save_HDF5_toNewDataBaseWiDefaultName(this)
        implicit none
        class(EquallySpacedSignal) :: this
    ...
    subroutine save_HDF5_toExistingDataBase(this,groupIndex)
        ...
        errorj=h5a_setType(groupIndex,'marmo:containers:EquallySpacedSignal' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of marmo:containers:EquallySpacedSignal'
            call throw(io_exception(error_message))
            return
        end if
        ...
        if (allocated(this%value)) then
            ...
            if (sv.ne.0) then
                errorj=-1
                error_message = 'Error during saving of EquallySpacedSignal, error when trying t&
                    &o allocate diml array for value'
        ...
        !-------------------------------------------------------------------------
        ! Save property xdelta
        !-------------------------------------------------------------------------
        errorj = H5A_WriteDouble(groupIndex, 'xdelta' // c_null_char,this%xdelta)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of xdelta'
            call throw(io_exception(error_message))
            return
        end if
        
        !-------------------------------------------------------------------------
        ! Save property xstart
        !-------------------------------------------------------------------------
        errorj = H5A_WriteDouble(groupIndex, 'xstart' // c_null_char,this%xstart)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of xstart'
            call throw(io_exception(error_message))
            return
        end if
        ...
        errorj = h5a_setOrder(groupIndex,'value,xname,xdescription,xunit,xdelta,xstart,x&
            &label,label,legend,unit,name,description' // c_null_char)
        if (H5A_IS_ERROR(errorj)) then
            error_message = 'Error during saving of description'
            call throw(io_exception(error_message))
            return
    ...
    subroutine load_HDF5_fromGroupIndex(this, groupIndex)
        ...
        if (sv.ne.0) then
            error=-1
            error_message = 'Error during loading of EquallySpacedSignal, error when trying &
                &to allocate diml array for value'
            call throw(illegal_state_exception(error_message%toChars()))
            return
        end if
        ...
        if (error >= 0) then
            ...
            if (sv.ne.0) then
                error=-1
                error_message = 'Error during loading of EquallySpacedSignal, error when trying &
                    &to allocate array for value'
                call throw(illegal_state_exception(error_message%toChars()))
                return
            end if
        error = H5A_ReadDoubleArray(groupIndex,'value' // c_null_char, this%value)
        if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of EquallySpacedSignal' +        &
        ...
        if (error.ge.0) then
            ...
            if (sv.ne.0) then
                error=-1
                error_message = 'Error during loading of EquallySpacedSignal, error when trying &
                    &to allocate name for xdescription'
                call throw(illegal_state_exception(error_message%toChars()))
                return
            end if
        ...
        !-------------------------------------------------------------------------
        ! Load property xdelta
        !-------------------------------------------------------------------------
        error = H5A_ReadDouble(groupIndex, 'xdelta' // c_null_char, this%xdelta)
        if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of EquallySpacedSignal'        &
                    + ' - failed to open property xdelta'
            call throw(io_exception(error_message))
            return
        end if
        !-------------------------------------------------------------------------
        ! Load property xstart
        !-------------------------------------------------------------------------
        error = H5A_ReadDouble(groupIndex, 'xstart' // c_null_char, this%xstart)
        if (H5A_IS_ERROR(error)) then
            error_message = 'Error during loading of EquallySpacedSignal'        &
                    + ' - failed to open property xstart'
            call throw(io_exception(error_message))
            return
        end if
        !-------------------------------------------------------------------------
    ...
    logical elemental function isValid(this)
        class(EquallySpacedSignal), intent(in) :: this
        isValid = .true.
    ...
    subroutine resize_value(this, n0)
        ...
        if (sv.ne.0) then
            error=-1
            error_message = 'Error during resizing in EquallySpacedSignal, error when trying&
                & to allocate memory for value'
            call throw(illegal_state_exception(error_message%toChars()))
            return
        end if
...
!---------------------------------------------------------------------------
end module  class_marmo_containers_EquallySpacedSignal
!******************************************************************************